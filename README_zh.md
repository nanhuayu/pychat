<div align="center">
   ...
   <p>
      ...
      PyChat 将 <strong>Chat</strong>、<strong>Agent</strong> 与 <strong>Tools</strong>
      整合到同一个桌面应用中，适合真实工作流的一站式 AI 使用体验。
   </p>
...
PyChat Agent 是一款面向桌面场景的一体化 AI 客户端，将 LLM 聊天、智能体工作流与工具调用整合进同一个 PyQt6 应用，同时保留 Python 技术栈易于扩展与维护的优势。

它适合以下使用方式：

- 在一个界面中统一管理多个模型供应商。
- 在同一桌面应用中统一使用 Chat、Agent 与 Tools。
...
| 模块 | 说明 |
| --- | --- |
| Chat / Agent / Tools 一体化 | 将传统聊天、智能体执行与工具调用统一到同一套桌面工作流中。 |
...
- 统一接入主流云端与本地模型。
- 让 Chat、Agent、Tools 共享一套桌面端交互体验，而不是分散在多个工具里。
- 支持更适合桌面端使用的会话管理方式。
...
如果你希望生成基于 Nuitka 的 Windows 独立运行包与 zip 压缩包，可执行：
...
- 使用 Nuitka 生成 Windows 独立运行目录，
- 打包 `assets/`、`pycat.ico`、`LICENSE`、`README.md` 与 `README_zh.md`，
- 在项目根目录生成 `PyChat-Agent-windows-x64.zip`。
...
PyChat 采用 GNU Affero General Public License v3.0（AGPL-3.0）开源协议。
